...
pub fn start_curio_daemon(
    ...
    start_curio_container(context, &container_name, builder)?;
...
fn start_curio_container(
    ...
    // connect to lotus network before starting
    ...
    let _ = run_and_log_command_strings("docker", &network_args, context, &key);
...
fn build_curio_builder(
    ...
    let mut builder = ContainerRunBuilder::create()
        .name(container_name)
        .network(&pdp_miner_network_name(run_id, sp_index))
        .port(api_port, 12300)
